...
namespace shopAcc.ApiIntegration
{
    public class UserApiClient : IUserApiClient
    {
        ...
        public async Task<ApiResult<string>> LoginMember(LoginRequest request)
        {
            var json = JsonConvert.SerializeObject(request);
            var httpContent = new StringContent(json, Encoding.UTF8, "application/json");
            ...
            var client = _httpClientFactory.CreateClient();
            client.BaseAddress = new Uri(_configuration["BaseAddress"]);
            var response = await client.PostAsync("/api/users/loginmenber", httpContent);
            if (response.IsSuccessStatusCode)
            {
                return JsonConvert.DeserializeObject<ApiSuccessResult<string>>(await response.Content.ReadAsStringAsync());
            }
        ...
        public async Task<ApiResult<UserVm>> GetByName(string userName)
        {
            var client = _httpClientFactory.CreateClient();
            client.BaseAddress = new Uri(_configuration["BaseAddress"]);
            var response = await client.GetAsync($"/api/users/search/{userName}");
            var body = await response.Content.ReadAsStringAsync();
            if (response.IsSuccessStatusCode)
                return JsonConvert.DeserializeObject<ApiSuccessResult<UserVm>>(body);
        ...
        public async Task<ApiResult<bool>> UpdateMem(Guid id, UserMemUpdateRequest request)
        {
            ...
            var response = await client.PutAsync($"/api/users/mem/{id}", httpContent);
            var result = await response.Content.ReadAsStringAsync();
            if (response.IsSuccessStatusCode)
                return JsonConvert.DeserializeObject<ApiSuccessResult<bool>>(result);
        ...
        public async Task<ApiResult<bool>> ChangePassWord(Guid id, ChangePassWordRequest request)
        {
            ...
            var response = await client.PutAsync($"/api/users/change/{id}", httpContent);
            var result = await response.Content.ReadAsStringAsync();
            if (response.IsSuccessStatusCode)
                return JsonConvert.DeserializeObject<ApiSuccessResult<bool>>(result);
